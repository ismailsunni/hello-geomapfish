...
RUN \
    ...
    do \
        node /usr/bin/compile-catalog \
            ...
            /tmp/config/geoportal/hello_geomapfish_geoportal/locale/${lang}/LC_MESSAGES/hello_geomapfish_geoportal-client.po \
            > /tmp/config/geoportal/hello_geomapfish_geoportal/static/${lang}.json; \
    done && \
    rm -rf /tmp/config/geoportal/hello_geomapfish_geoportal/locale

RUN \
    cd /tmp/config/geoportal/ && \
    c2c-template --vars ${VARS_FILE} \
        --get-config hello_geomapfish_geoportal/config.yaml \
        ${CONFIG_VARS} && \
    pykwalify --data-file hello_geomapfish_geoportal/config.yaml \
        --schema-file CONST_config-schema.yaml && \
    rm CONST_* vars.yaml

###############################################################################

FROM camptocamp/geomapfish-config:2.5.0
...
RUN \
    ...
    mv /tmp/config/geoportal/hello_geomapfish_geoportal/ /etc/geomapfish/ && \
