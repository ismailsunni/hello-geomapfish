...
for (const filename of ls(path.resolve(__dirname, 'hello_geomapfish_geoportal/static-ngeo/js/apps/*.html.ejs'))) {
  const name = filename.file.substr(0, filename.file.length - '.html.ejs'.length);
  entry[name] = 'hello_geomapfish/apps/Controller' + name + '.js';
  plugins.push(
...
const projectRule = {
  test: /hello_geomapfish_geoportal\/static-ngeo\/js\/.*\.js$/,
  use: {
    loader: 'babel-loader',
    options: {
...
module.exports = {
  ...
  resolve: {
    ...
    alias: {
      hello_geomapfish: path.resolve(__dirname, 'hello_geomapfish_geoportal/static-ngeo/js'),
    },
  },
};
